...
`quantity` - Decimal index of the starting block to fix.
If left empty, the head block is used as the starting point.
...
* `enode` - link:../Concepts/Node-Keys.md#enode-url[Enode URL] of the node.
...
The remote address might not match the hex value for `port`.
...
* `protocols` - link:../HowTo/Find-and-Connect/Managing-Peers.md#monitoring-peer-connections[Current state of peer] including `difficulty` and `head`.
`head` is the hash of the highest known block for the peer.
...
Returns the xref:../Concepts/NetworkID-And-ChainID.adoc[network ID].
...
| Network ID | Chain | Network | Description |--------|-----|------|---------------------| | `1` | ETH | Mainnet | Main Ethereum network | | `3` | ETH | Ropsten | PoW test network | | `4` | ETH | Rinkeby | PoA test network using Clique | | `5` | ETH | Goerli | PoA test network using Clique | | `2018` | ETH | Dev | PoW development network | | `1` | ETC | Classic | Main Ethereum Classic network | | `6` | ETC | Kotti | PoA test network using Clique | | `7` | ETC | Mordor | PoW test network |
...
`result` : _string_ - link:../Concepts/Node-Keys.md#enode-url[Enode URL] of the node.
...
`result` : _Object|Boolean_ - Object with synchronization status data or `false` if not synchronizing:
...
* `pulledStates`: _quantity_ - If fast synchronizing, the number of state entries fetched so far, or `null` if this is not known or not relevant.
If full synchronizing or fully synchronized, this field is not returned.
* `knownStates`: _quantity_ - If fast synchronizing, the number of states the node knows of so far, or `null` if this is not known or not relevant.
If full synchronizing or fully synchronized, this field is not returned.
...
Whether the client is actively mining new blocks.
Besu pauses mining while the client synchronizes with the network regardless of command settings or methods called.
...
`result` (_BOOLEAN_) - `true` if the client is actively mining new blocks, otherwise `false`.
...
By default, the last 100 blocks are examined and the 50th percentile gas unit price (that is, the median value) is returned.

If there are no blocks, the value for link:CLI/CLI-Syntax.md#min-gas-price[`--min-gas-price`] is returned.
The value returned is restricted to values between link:CLI/CLI-Syntax.md#min-gas-price[`--min-gas-price`] and link:CLI/CLI-Syntax.md#api-gas-price-max[`--api-gas-price-max`].
By default, 1000 Wei and 500GWei.

Use the link:CLI/CLI-Syntax.md#api-gas-price-blocks[`--api-gas-price-blocks`], link:CLI/CLI-Syntax.md#api-gas-price-percentile[`--api-gas-price-percentile`] , and link:CLI/CLI-Syntax.md#api-gas-price-max[`--api-gas-price-max`] command line options to configure the `eth_gasPrice` default values.
...
`result`: `object` - link:API-Objects.md#miner-data-object[Miner data].
...
Use the `pending` tag to get the next account nonce not used by any pending transactions.
...
Set the maximum transaction fee for transactions using the link:CLI/CLI-Syntax.md#rpc-tx-feecap[`--rpc-tx-feecap`] CLI option.
...
_QUANTITY|TAG_ - Integer representing a block number or one of the string tags `latest`, `earliest`, or `pending`, as described in link:../HowTo/Interact/APIs/Using-JSON-RPC-API.md#block-parameter[Block Parameter].
...
The resulting estimate can be greater than the amount of gas the transaction ends up using, for reasons including EVM mechanics and node performance.

The `eth_estimateGas` call does not send a transaction.
You must call <<eth_sendrawtransaction,`eth_sendRawTransaction`>> to execute the transaction.
...
`result` : _OBJECT_ - link:API-Objects.md#block-object[Block object] , or `null` when there is no block.
...
Object - link:API-Objects.md#transaction-object[Transaction object], or `null` when there is no transaction.
...
If you enabled xref:../HowTo/Send-Transactions/Revert-Reason.adoc[revert reason], the receipt includes available revert reasons in the response.
...
`Object` - link:API-Objects.md#transaction-receipt-object[Transaction receipt object], or `null` when there is no receipt.
...
* `DATA`, 32 Bytes - The required target boundary condition: 2{caret}256 / difficulty.
...
Propose to link:../HowTo/Configure/Consensus-Protocols/Clique.md#adding-and-removing-signers[add or remove a signer with the specified address].
...
* `nextKey`:``data`` - Hash of the next address if any addresses remain in the state, otherwise zero.
...
Sends a list of xref:../HowTo/Send-Transactions/Transactions.adoc[signed transactions].
...
Generates files containing the block trace.
A separate file is generated for each transaction in the block.
...
Optional.
If omitted, then a trace file is generated for each transaction in the block.
...
Use the _Debugger_ tab in Remix instead of calling `debug_storageRangeAt` directly.
...
Use the _Debugger_ tab in Remix instead of calling `debug_traceTransaction` directly.
...
Updates the target gas limit set using the link:CLI/CLI-Syntax.md#target-gas-limit[`--target-gas-limit`] command line option.
...
To start mining, you must first specify a miner coinbase using the link:CLI/CLI-Syntax.md#miner-coinbase[`--miner-coinbase`] command line option.
...
Returns link:../HowTo/Configure/Consensus-Protocols/IBFT.md#adding-and-removing-validators[votes] cast in the current link:../HowTo/Configure/Consensus-Protocols/IBFT.md#genesis-file[epoch].
...
Propose to link:../HowTo/Configure/Consensus-Protocols/IBFT.md#adding-and-removing-validators[add or remove a validator] with the specified address.
...
Use the permissioning API methods for xref:../HowTo/Limit-Access/Local-Permissioning.adoc[local] permissioning only.
...
Adds accounts (participants) to the link:../HowTo/Limit-Access/Local-Permissioning.md#account-permissioning[accounts permission list].
...
`result` - `Success` or `error`.
Errors include attempting to add accounts already on the allowlist or including invalid account addresses.
...
Lists accounts (participants) in the link:../HowTo/Limit-Access/Local-Permissioning.md#account-permissioning[accounts permissions list].
...
Removes accounts (participants) from the link:../HowTo/Limit-Access/Local-Permissioning.md#account-permissioning[accounts permissions list].
...
`result` - `Success` or `error`.
Errors include attempting to remove accounts not on the allowlist or including invalid account addresses.
...
Adds nodes to the link:../HowTo/Limit-Access/Local-Permissioning.md#node-allowlisting[nodes allowlist].
...
Lists nodes in the link:../HowTo/Limit-Access/Local-Permissioning.md#node-allowlisting[nodes allowlist].
...
`result: list` - link:../Concepts/Node-Keys.md#enode-url[Enode URLs] of nodes in the nodes allowlist.
...
Removes nodes from the link:../HowTo/Limit-Access/Local-Permissioning.md#node-allowlisting[nodes allowlist].
...
`result` - `Success` or `error`.
Errors include attempting to remove nodes not on the allowlist or including invalid enode URLs.
...
Reloads the accounts and nodes allowlists from the link:../HowTo/Limit-Access/Local-Permissioning.md#permissions-configuration-file[permissions configuration file].
...
| Field name | Value | Value type | Supported operators | |----------|-----------------------------|:--------------:|--------------| | *from* | Address of the sender.
| _Data_, 20 bytes | `eq` | | *to* | Address of the receiver, or `"contract_creation"`.| _Data_, 20 bytes |`eq`, `action`| | *gas* | Gas provided by the sender.
| _Quantity_ | `eq`, `gt`, `lt` | | *gasPrice* | Gas price, in wei, provided by the sender.| _Quantity_ | `eq`, `gt`, `lt` | | *value* | Value transferred, in wei.
| _Quantity_ | `eq`, `gt`, `lt` | | *nonce* | Number of transactions made by the sender.| _Quantity_ | `eq`, `gt`, `lt` | |
...
`result` - Array of objects with link:API-Objects.md#pending-transaction-object[details of the pending transaction].
...
`array of strings` - Tracing options are xref:Trace-Types.adoc[`trace`, `vmTrace`, and `stateDiff`].
Specify any combination of the three options including none of them.

==== Returns

`result` - Array of link:API-Objects.md#transaction-trace-object[transaction trace objects] containing one object per transaction, in transaction execution order.
...
Provides transaction processing of link:Trace-Types.md#trace[type `trace`] for the specified block.
...
`result` - Array of link:Trace-Types.md#trace[calls to other contracts] containing one object per call, in transaction execution order.
...
Provides transaction processing of link:Trace-Types.md#trace[type `trace`] for the specified transaction.
...
`result` - Array of link:Trace-Types.md#trace[calls to other contracts] containing one object per call, in the order called by the transaction.
...
The `gas` and `gasPrice` are used by the xref:../Concepts/Privacy/Private-Transaction-Processing.adoc[privacy marker transaction] not the private transaction itself.
...
The enclave key is a pointer to the private transaction in https://docs.orion.consensys.net/[Orion].
...
Filters for private contracts can only be created by <<priv_newfilter,`priv_newFilter`>> so unlike <<eth_getfilterchanges,`eth_getFilterChanges`>>, `priv_getFilterChanges` always returns an array of log objects or an empty list.
...
`array` - link:API-Objects.md#log-object[Log objects].
If nothing has changed since the last poll, an empty list.
...
Returns an array of xref:../Concepts/Events-and-Logs.adoc[logs] for the specified filter for a private contract.

For private contracts, `priv_getFilterLogs` is the same as <<eth_getfilterlogs,`eth_getFilterLogs`>> for public contracts except there is no link:CLI/CLI-Syntax.md#auto-log-bloom-caching-enabled[automatic log bloom caching] for private contracts.
...
For private contracts, `priv_getLogs` is the same as <<eth_getlogs,`eth_getLogs`>> for public contracts except there is no link:CLI/CLI-Syntax.md#auto-log-bloom-caching-enabled[automatic log bloom caching] for private contracts.
...
The address is derived and based on the value of the link:CLI/CLI-Syntax.md#privacy-onchain-groups-enabled[`privacy-onchain-groups-enabled`] option.
...
`data` - Transaction hash returned by <<eea_sendrawtransaction,`eea_sendRawTransaction`>> or https://docs.ethsigner.consensys.net/en/latest/Using-EthSigner/Using-EthSigner/#eea_sendtransaction[`eea_sendTransaction`].
...
Creates a group of nodes, specified by their https://docs.orion.consensys.net/[Orion] public key.

==== Parameters

`Object` - Request options:

* `addresses`: `array of data` - Array of nodes, specified by https://docs.orion.consensys.net/[Orion] public keys.
...
`array of data` - Members specified by https://docs.orion.consensys.net/[Orion] public keys.
...
Creates a xref:../Concepts/Events-and-Logs.adoc[log filter] for a private contract.
...
For private contracts, `priv_newFilter` is the same as <<eth_newfilter,`eth_newFilter`>> for public contracts.
...
Uninstalls a filter for a private contract with the specified ID.
When a filter is no longer required, call this method.
...
For private contracts, `priv_uninstallFilter` is the same as <<eth_uninstallfilter,`eth_uninstallFilter`>> for public contracts.
